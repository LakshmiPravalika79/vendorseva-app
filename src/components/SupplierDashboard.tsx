...
export default function SupplierDashboard() {
  ...
  const handleImageUpload = async (): Promise<string | null> => {
    ...
    // --- PASTE YOUR REAL IMGBB API KEY HERE ---
    const IMGBB_API_KEY = "c5e9ffce10c619856ed3aacec528de6d"; 
    ...
    try {
      ...
      toast({ title: "Image Upload Failed", description: "Please check your API key and try again.", variant: "destructive" });
      return null;
    } finally {
  ...
  const handleUpdateOrderStatus = async (orderId: string, status: 'accepted' | 'declined') => { const orderRef = doc(db, 'orders', orderId); try { await updateDoc(orderRef, { status: status }); toast({ title: `Order ${status}` }); } catch (error) { console.error(`Error updating order: `, error); toast({ title: "Error", variant: "destructive" }); } };
  const handleShowVendorDetails = async (vendorId: string) => { const vendorDocRef = doc(db, 'users', vendorId); try { const docSnap = await getDoc(vendorDocRef); if (docSnap.exists()) { const vendorData = docSnap.data() as DocumentData; setCurrentVendor({ email: vendorData.email, address: vendorData.address }); setIsVendorDetailsOpen(true); } else { toast({ title: "Error", description: "Could not find vendor details.", variant: "destructive" }); } } catch (error) { console.error("Error fetching vendor details: ", error); toast({ title: "Error", description: "Failed to fetch details.", variant: "destructive" }); } };
  const getStatusColor = (status: string) => { switch (status) { case 'pending': return 'bg-yellow-100 text-yellow-800'; case 'accepted': return 'bg-green-100 text-green-800'; case 'declined': return 'bg-red-100 text-red-800'; default: return 'bg-gray-100 text-gray-800'; } };
