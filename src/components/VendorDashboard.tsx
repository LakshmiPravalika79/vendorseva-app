...
import { collection, getDocs, addDoc, query, where } from 'firebase/firestore'; // <-- Added 'query' and 'where'
...
export default function VendorDashboard() {
  ...
  // UPDATED: Fetch only 'available' products
  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true);
      try {
        const productsRef = collection(db, 'products');
        // This query now only gets products that are available
  ...
  // ... (filter and order functions are unchanged) ...
  useEffect(() => {
  ...
  const handleOrderNow = async (product: Product) => {
    ...
    try { await addDoc(collection(db, 'orders'), { vendorId: user.uid, supplierId: product.supplierId, productName: product.name, totalAmount: product.pricePerUnit, status: 'pending', createdAt: new Date(), }); toast({ title: "✅ Order Placed!", description: `Your order for ${product.name} has been sent.` }); } catch (error) { console.error("Error placing order: ", error); toast({ title: "Error", description: "Could not place order.", variant: "destructive" }); }
  };
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {filteredProducts.map((product) => (
              <Card key={product.id} className="flex flex-col overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                <div className="aspect-video bg-gray-100 relative"> {/* Added relative positioning */}
                ...
                <div className="p-4 flex flex-col flex-grow">
                    ...
                    <CardContent className="p-0 flex-grow flex flex-col justify-end">
                        ...
                        {/* UPDATED: Button is disabled if unavailable */}
